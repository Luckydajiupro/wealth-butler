...
graph TB
    ...
    subgraph Rules层["Rules 规则引擎层（WealthButler/Rules/）"]
        R["ruleEngine.py<br/>15条风控规则定义与评估<br/>8条实时/准实时 + 7条日批/周批"]
    end
    ...
    subgraph 数据层["Base/Client 层（脚手架复用）"]
        ...
        Milvus[(Milvus<br/>3个集合：FAQ/产品/政策)]
...
10. **Rules层**（规则引擎层）：15条风控规则的定义、评估、触发逻辑
11. **Utils层**（业务工具层）：纯函数式工具（金融计算、格式化、校验）

**与需求文档的关系**：Agent职责/服务对象/触发方式的具体文字定义见§2.2；EventBus的`publish()`/`consume()`实现代码见§2.4；中间件改造点（新增`MemoryRecallMiddleware`）见§2.3；统一入口与直连路由的完整清单见§7.1；编排层的落地方式见ADR-6；10个Tool的Schema定义见Agent设计文档§7；5段式Prompt模板见Agent设计文档§1.2；15条规则清单见用户研判规则/目录。
...
- **业务实体**：10张业务表（wealth_*前缀）、3个Milvus集合、Neo4j知识图谱
- **核心功能**：5个智能Agent、10个BaseTool实现、15条风控规则、RAG/GraphRAG管道
...
|---|---|---|---|
| MySQL | `Base/Client/mysqlClient.py` | `Base/Repository/base/baseDBModel.py` | 10张业务表Model：`WealthButler/Models/` |
| Milvus | `Base/Client/milvusClient.py` | `Base/Repository/base/baseVDB.py` | 3个集合入库管道：`WealthButler/Knowledge/ragIngestion.py` |
